...
var GridWorldPainter = function (context) {
  ...
  this.AGENT_WIDTH = this.TILE_SIZE * 0.3;
  this.WALL_STROKE_SIZE = 10;
  this.ACTION_ANIMATION_TIME = 150;
  this.CONTRACT_SIZE = 0.9;
  this.SUBACTION_PROPORTION = 0.2;
  this.SUBACTION_DISTANCE = 0.2;
  this.SUBACTION_COLLISION_PROPORTION = 0.7;
  this.SUBACTION_COLLISION_DISTANCE = 0.7;
...
GridWorldPainter.prototype.init = function (container) {
      ...
      for (var w = 0; w < this.gridworld.walls.length; w++) {
        ...
        if (String([wall[0], wall[1]]) == String([x, y])) {
          ...
          if (wall.length == 2) {
            ...
            from = [
              (x + from[0]) * this.TILE_SIZE,
              (this.gridworld.height - (from[1] + y)) * this.TILE_SIZE,
            ];
            to = [
              (x + to[0]) * this.TILE_SIZE,
              (this.gridworld.height - (to[1] + y)) * this.TILE_SIZE,
            ];

            var wall_path = "M" + from.join(" ") + "L" + to.join(" ");
            var wall_img = this.paper.path(wall_path);
            wall_img.attr({
              "stroke-width": this.WALL_STROKE_SIZE,
              stroke: "black",
            });
          }
        }
  ...
  if (this.draw_goals) {
    for (g = 0; g < this.gridworld.goals.length; g++) {
      ...
      var params = {
        type: "text",
        x: (goal.location[0] + 0.5) * this.TILE_SIZE,
        y: (this.gridworld.height - goal.location[1] - 0.5) * this.TILE_SIZE,
...
GridWorldPainter.prototype.drawState = function (state) {
  ...
  for (var object in state) {
    ...
    if (object.type == "agent") {
      var agent_params = {
        cx: (object.location[0] + 0.5) * this.TILE_SIZE,
        cy: (this.gridworld.height - object.location[1] - 0.5) * this.TILE_SIZE,
      };
...
GridWorldPainter.prototype.drawTransition = function (
  state,
  action,
  nextState,
  mdp
  ...
  for (agent in state) {
    if (state.hasOwnProperty(agent)) {
      ...
      if (typeof mdp == "undefined") {
        ...
        intendedLocation[agent] = mdp.getNextIntendedLocation(
  ...
  for (agent in state) {
    ...
    if (action[agent] == "wait") {
      ...
      var expand = (function (painter, agentImage, original_size, time) {
        ...
        this,
        this.objectImages[agent],
        this.objectImages[agent].attr("r"),
        this.ACTION_ANIMATION_TIME * 0.5
      );

      var contract = Raphael.animation(
        { r: this.objectImages[agent].attr("r") * this.CONTRACT_SIZE },
        this.ACTION_ANIMATION_TIME * 0.5,
        "backIn",
        expand
      );
      ...
      $.subscribe(
        "killtimers",
        this.makeTimerKiller(this.objectImages[agent], contract)
      );
    }
    //try and fail
    else if (
      ...
      for (var otherAgent in state) {
        if (
          !state.hasOwnProperty(otherAgent) ||
          state[otherAgent].type !== "agent" ||
          agent == otherAgent
        ) {
          continue;
        }
        ...
        if (
          (String(intendedLocation[agent]) ==
            String(nextState[otherAgent].location) ||
            String(intendedLocation[agent]) ==
              String(intendedLocation[otherAgent])) &&
          !(
            String(intendedLocation[agent]) ==
              String(nextState[otherAgent].location) &&
            String(intendedLocation[otherAgent]) ==
      ...
      var moveBack = (function (
        ...
        this,
        this.objectImages[agent],
        this.objectImages[agent].attr("cx"),
        this.objectImages[agent].attr("cy"),
        this.ACTION_ANIMATION_TIME * SUBACTION_PROPORTION
      );

      var new_x =
        intendedLocation[agent][0] * SUBACTION_DISTANCE +
        state[agent].location[0] * (1 - SUBACTION_DISTANCE);

      var new_y =
        (this.gridworld.height - intendedLocation[agent][1]) *
          SUBACTION_DISTANCE +
        (this.gridworld.height - state[agent].location[1]) *
          (1 - SUBACTION_DISTANCE);

      new_x = (new_x + 0.5) * this.TILE_SIZE;
      new_y = (new_y - 0.5) * this.TILE_SIZE;

      var moveToward = Raphael.animation(
        { cx: new_x, cy: new_y },
        this.ACTION_ANIMATION_TIME * (1 - SUBACTION_PROPORTION),
        "backIn",
        moveBack
      );
      ...
      $.subscribe(
        "killtimers",
        this.makeTimerKiller(this.objectImages[agent], moveToward)
      );
    }
    //normal movement
    else {
      ...
      var movement = Raphael.animation(
        {
          ...
          cy:
            (this.gridworld.height - nextState[agent].location[1] - 0.5) *
            this.TILE_SIZE,
        },
        this.ACTION_ANIMATION_TIME,
        "easeInOut"
      );
      this.objectImages[agent].animate(movement);
      $.subscribe(
        "killtimers",
        this.makeTimerKiller(this.objectImages[agent], movement)
      );
    }
  }
};
...
GridWorldPainter.prototype.showReward = function (loc, agent, text) {
  var params = {
    type: "text",
    x: (loc[0] + 0.5) * this.TILE_SIZE,
    y: (this.gridworld.height - loc[1] - 0.5) * this.TILE_SIZE,
    text: text,
    "font-size": 30,
    "font-weight": "bold",
    stroke: "black",
  ...
  var r = this.paper.add([params])[0];
  var r_animate = Raphael.animation(
    { y: r.attr("y") - 0.5 * this.TILE_SIZE, opacity: 0 },
    this.REWARD_DISPLAY_TIME
  );
