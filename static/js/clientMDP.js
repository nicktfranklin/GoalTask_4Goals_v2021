...
ClientMDP.prototype.getTransition = function (state, action) {
  ...
  for (var agent in state) {
    ...
    nextIntendedLocations[agent] = this.getNextIntendedLocation(
  ...
  for (var agent in nextIntendedLocations) {
    if (
      nextIntendedLocations[agent][0] >= this.gridworld.width ||
    ...
    for (var w = 0; w < this.gridworld.walls.length; w++) {
      ...
      if (
        wall.length == 2 &&
        String(wall) == String(nextIntendedLocations[agent])
      ) {
        nextState[agent] = state[agent];
      }
      //1D walls
      if (
        wall.length == 3 &&
        String(wall.slice(0, 2)) == String(state[agent].location) &&
        wall[2] == action[agent]
      ) {
        nextState[agent] = state[agent];
      }
  ...
  for (var agent in nextIntendedLocations) {
    for (var oldagent in nextState) {
      ...
      if (
        String(nextIntendedLocations[agent]) ==
        String(nextState[oldagent].location)
      ) {
    ...
    for (var oldagent in state) {
      if (
        String(nextIntendedLocations[agent]) == String(state[oldagent].location)
      ) {
        cannotMove[agent] = state[agent];
      }
    }
  ...
  for (var agent in nextIntendedLocations) {
    if (typeof nextState[agent] == "undefined") {
      nextState[agent] = {
        name: agent,
        location: nextIntendedLocations[agent],
        type: "agent",
      };
    }
  }
