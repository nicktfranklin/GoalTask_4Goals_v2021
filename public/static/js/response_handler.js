...
demo_responseHandlerGenerator = function (action_mapping) {
  return function (event) {
    ...
    for (var agent in this.state) {
      if (this.state.hasOwnProperty(agent)) {
        if (this.mdp.inGoal(nextState[agent]["location"], agent)) {
          var goal_value = this.mdp.getStateValue(
            nextState[agent]["location"],
            agent
          );
          var display;
    ...
    th = setTimeout(
      reset_key_handler,
      this.painter.ACTION_ANIMATION_TIME + wait_before_actions_time
    );
    $.subscribe(
      "killtimers",
      (function (th) {
...
var demo_responseHandler_generator_noReachableAction = function (
  action_mapping
) {
  // allow_response = true;
  var n_responses = 0;
  ...
  return function (event) {
    ...
    if (n_responses >= 7) {
      $("#trial_text").html(
        'Great!<br> <br> <I><span style="color: #707070">Press enter to continue</span></br>'
      );
      move_to_next_trial = true;
    }
...
var demo_responseHandler_generator_endDemo = function (action_mapping) {
  ...
  return function (event) {
    ...
    if (n_responses >= 4) {
      $("#trial_text").html(
        "That's how it works!!!<br>.~*`*~.~*`*~.~*`*~.~*`*~.<br> <I><span style='color: #707070'>Press enter to continue</span></br>"
      );
    }
  ...
  return function (event) {
    ...
    for (var agent in this.state) {
      if (this.mdp.inGoal(nextState[agent]["location"], agent)) {
        ...
        goal_value = this.mdp.getStateValue(
          nextState[agent]["location"],
          agent
        );
        goal_display_label = this.mdp.getGoalDisplayLabel(
          nextState[agent]["location"],
          agent
        );
        ...
        var celebrateGoal = (function (
          ...
          return function () {
            ...
            var text_display =
              ...
              '<span style="color:' +
              painter.AGENT_COLORS["agent1"] +
              '">' +
              "Room " +
              String(context) +
              '</span></span></span><br> and won: <span style="color:' +
              painter.AGENT_COLORS["agent1"] +
        ...
        var text_display =
          ...
          '<span style="color:' +
          this.painter.AGENT_COLORS["agent1"] +
          '">' +
