...
function generate_randomized_blocks(context_balance, block_size) {

  // generate a bag of contexts and randomize the order
  var bag_of_contexts = [];
...
function random_goal_locations() {
  ...
  var loc_ids = [
    quarter_1_goal_ids.pop(),
    quarter_2_goal_ids.pop(),
    quarter_3_goal_ids.pop(),
    quarter_4_goal_ids.pop(),
  ];
...
function random_initial_locations() {
  ...
  while (true) {
    ...
    for (ii = 0; ii < goal_locations.length; ii++) {
      ...
      var dist =
        Math.abs(g_loc[0] - init_loc[0]) + Math.abs(g_loc[1] - init_loc[1]);
